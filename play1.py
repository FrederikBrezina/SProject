...
def train_model(dimension_of_decoder, num_of_act_fce1, min_units1, max_units1,min_depth1, max_depth,
    ...
    for epoch in range(0,500):
        train_on_epoch(full_model, datax_hidden[:-10], datax_hidden_t[:-10], datax_fce[:-10], datax_fce_t[:-10], epoch, batch_size=10,
                       reverse_order=reverse_order)
        print("val_loss____________________________",full_model.test_on_batch([datax_hidden[-10:], datax_fce[-10:]],[datax_hidden_t[-10:], datax_fce_t[-10:]]))
    print(full_model.predict([datax_hidden[:2], datax_fce[:2]]))
    print("33333333333333333333333333333333333333333333333")
    print([datax_hidden[:2], datax_fce[:2]])
...
train_model(64, 8, 2, 100,2, 11,
                1000, 9, reverse_order=True)